        ...
        with st.expander(exp_title, expanded=False):
            if display_mode == "Tables":
                ...
                if n_peers > 0 and present_cols:
                    ...
                    peers_yes_mean   = float(yes_p.sum())
                ...
                if peers_yes_mean is not None:
                    ...
                    rows += [
                        {"Series": peers_label, "Category": "Yes",   "Value": peers_yes_mean,   "Total": len(present_cols)},
                        {"Series": peers_label, "Category": "No",    "Value": peers_no_mean,    "Total": len(present_cols)},
                        {"Series": peers_label, "Category": "Blank", "Value": peers_blank_mean, "Total": len(present_cols)},
                    ]
                ...
                # Child 1: stacked bars (no .properties here!)
                bars = (
                    base
                    .transform_calculate(CatOrder="{'Yes':0,'No':1,'Blank':2}[datum.Category]")
                    .mark_bar()
                    .encode(
                        ...
                        x=alt.X(
                            ...
                            axis=alt.Axis(values=list(range(0, len(present_cols)+1)),
                                          tickCount=len(present_cols)+1, format="d"),
                        ),
                        color=alt.Color(
                            "Category:N",
                            scale=alt.Scale(
                ...
                # Child 2: totals text (no .properties here!)
                totals = (
                    base
                    .transform_aggregate(total="sum(Value)", groupby=["Series"])
                    .mark_text(align="left", baseline="middle", dx=4)
                ...
                # Set top-level props on the LAYER, not on the children
