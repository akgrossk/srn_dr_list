...
if view == "Total":
    ...
    if display_mode == "Charts":
        ...
        if overview_rows:
            ...
            # header + inline legend (per pillar)
            render_section_header(f"{PILLAR_LABEL[pillar]} — Overview", present_codes)

            y_sort = [firm_series_label] + ([peers_series_label] if peers_series_label else [])
            base = alt.Chart(chart_df)

            # For G only, force integer ticks 0..N
            if pillar == "G":
            ...
            bars = (
                ...
                .encode(
                    ...
                    color=alt.Color("StdCode:N",
                                    scale=alt.Scale(domain=color_domain, range=color_range),
                                    legend=None),  # legend disabled (inline above)
                    order=alt.Order("StdCode:N", sort="ascending"),
                    tooltip=[
                        alt.Tooltip("Series:N", title="Series"),
                        alt.Tooltip("Standard:N", title="Standard"),
            ...
            st.altair_chart(fig, use_container_width=True)
            st.caption(
                "Two bars (Firm vs Peers). Segments are the standards in this pillar (E1–E5, S1–S4, G1)."
