# esg_app.py — DR Viewer with Country/Industry/Custom comparison
# Requires: streamlit, pandas, numpy, altair, requests, openpyxl
...
# ========= CONFIG =========
DEFAULT_DATA_PATH = r"C:\Users\agrosko\Dropbox\Coding\srn\DR_extract.xlsx"  # local dev only
# Auto-load this on deploy (blob URL is fine; we convert it to RAW internally)
...
def pretty_value(v):
    ...
    s = str(v).strip()
    low = s.lower()
    if low in YES_SET: return "✅ Yes"
    if low in NO_SET:  return "❌ No"
    return s
...
# ========= LOAD DATA (auto-first) =========
st.sidebar.title("🌱 DR Viewer")

df = pd.DataFrame()
auto_url = read_query_param("data_url", None)
if not auto_url:
    try:
...
if auto_url:
    ...
    if not df.empty:
        st.sidebar.caption(f"Loaded data from URL.")
    else:
        st.sidebar.warning("Auto-load failed; use URL or Upload below.")

if df.empty:
    # Fallback loaders for troubleshooting / dev
    source_mode = st.sidebar.radio("Data source", ["URL", "Upload", "Local path (dev)"], index=0)
    if source_mode == "URL":
        data_url = st.sidebar.text_input("Data URL (.xlsx or .csv)", value=auto_url or "", placeholder="Paste RAW GitHub/Dropbox/HTTPS link…")
        if data_url:
        ...
        df = load_table(DEFAULT_DATA_PATH)

if df.empty:
    st.info("No data loaded yet. Configure DATA_URL or pass ?data_url=... in the link.")
    st.stop()
...
if current_view not in valid_views: current_view = "Combined"
...
if view == "Combined":
    ...
    for pillar in ["E", "S", "G"]:
        ...
        chart_rows.append({"Pillar": PILLAR_LABEL[pillar], "Series": "Firm — # Yes", "Value": firm_yes, "Total": total_metrics, "Link": f"?view={pillar}"})
        if peer_yes_mean is not None:
            chart_rows.append({"Pillar": PILLAR_LABEL[pillar], "Series": f"Peers — mean # Yes ({comp_label})", "Value": round(peer_yes_mean, 1), "Total": total_metrics, "Link": f"?view={pillar}"})
    ...
    if not chart_df.empty:
        ...
        chart = (
            ...
            .encode(
                ...
                color=alt.Color("Pillar:N", scale=alt.Scale(domain=list(base_colors.keys()),
                                                            range=list(base_colors.values())),
                                legend=None),
                opacity=alt.Opacity("Series:N",
                                    scale=alt.Scale(domain=["Firm — # Yes", f"Peers — mean # Yes ({comp_label})"],
                                                    range=[1.0, 0.55]),
